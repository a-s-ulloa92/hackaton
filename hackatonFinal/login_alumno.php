...
if(isset($_POST['valida'])){
	#Checa que los campos de clave y nombre no esten vacios
	if(!empty($_POST['clave']) && !empty($_POST['nombre'])){

		$tnombre = $_POST['nombre'];
		$tclave = $_POST['clave'];

		#Ve si hay un examen en la BD con la clave dada. Si existe, ocurre
		#un error; si no, pasa a la aplicacion del examen
		...
		if ($res->num_rows < 1){
			...
			#Guarda ID del examen para usarlo despues
			...
			$cade = "insert into alumno(login, Examen_idExamen)
			values('".$tnombre."','".$_SESSION['idexamen']."')";
			...
			if($res) {
				...
				#Guarda ID del alumno para usarlo mas adelante.
